...
import 'package:mobileapp/widgets/intl_phone_number_input-0.7.0+2/src/utils/phone_number/phone_number_util.dart';
...
class _InputWidgetState extends State<InternationalPhoneNumberInput> {
  ...
  void loadCountries({Country? previouslySelectedCountry}) {
    if (this.mounted) {
      List<Country> countries = CountryProvider().reload(widget.countries);
        ...
        if (phoneNumber == null) {
          ...
          if (widget.onInputChanged != null) {
            try {
              ...
              log(e.toString());
              log(PhoneNumber(phoneNumber: phoneNumber, isoCode: this.country?.alpha2Code, dialCode: this.country?.dialCode).toString());
            }
          }
