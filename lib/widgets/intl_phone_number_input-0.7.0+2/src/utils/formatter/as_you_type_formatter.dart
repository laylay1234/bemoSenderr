  ...
  TextEditingValue formatEditUpdate(TextEditingValue oldValue, TextEditingValue newValue) {
    ...
    if (newValueLength > 0 && newValueLength > oldValueLength) {
      ...
      final _ = newValueText
          .substring(oldValue.selection.start == -1 ? 0 : oldValue.selection.start, newValue.selection.end == -1 ? 0 : newValue.selection.end)
      ...
      formatAsYouType(input: textToParse).then(
        (String? value) {
          ...
          if (separatorChars.hasMatch(parsedText)) {
            ...
            if (parsedText.length > offset) {
              valueInInputIndex = parsedText[offset];
            } else {
              try {
                valueInInputIndex = parsedText[offset - 1];
              } catch (e) {
                valueInInputIndex = parsedText[offset - 2];
              }
            }
